...
template <>
struct getter<ImVec2>
{
    static ImVec2 get(lua_State * L, int index, record & tracking)
    {
        sol::table oPos = sol::stack::get<sol::table>(L, lua_absindex(L, index), tracking);
        return { oPos[1], oPos[2] };
    }
};

template <>
struct pusher<ImVec2>
{
    static int push(lua_State * L, const ImVec2 & _vec)
    {
